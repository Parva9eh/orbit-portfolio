...
function SectionHead({
  ...
  return (
    <div className={`flex items-center justify-between gap-2 mb-1.5 border-l-2 pl-2 ${bar}`}>
      <h6 className={`text-[10px] uppercase tracking-widest font-semibold ${titleColor}`}>
        {title}
      </h6>
...
function AsteroidBrief({
  ...
  return (
    <div className="space-y-3 text-xs">
      {/* Legend — both sources are always “real”; not exclusive modes */}
          ...
          <dl className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-0.5 text-gray-300">
            ...
            {approach.orbitingBody && (
              <>
                ...
                <dd className="text-right text-gray-100">{approach.orbitingBody}</dd>
              </>
            )}
          ...
          <>
            {sbdb?.orbitClass && (
              <p className="text-gray-400 mb-1">
                Class · <span className="text-emerald-100/90">{sbdb.orbitClass}</span>
              </p>
            )}
            <dl className="grid grid-cols-[auto_1fr] gap-x-2 gap-y-0.5 text-gray-300">
              {aAu != null && (
          ...
          <>
            ...
            {sbdb && !sbdb.found && !sbdbLoading && (
              <p className="text-[10px] text-gray-500 mt-1.5">
                {sbdb.message ?? "Not found in SBDB"} — NeoWs approach still valid
              </p>
            )}
            {!sbdbLoading && !sbdbError && !sbdb && a.orbitSource !== "mock" && (
...
function PlanetBrief({ p }: { p: Planet }) {
  return (
